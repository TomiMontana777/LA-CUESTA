...
def buscadorNombre(request):
    query = request.GET.get('',Node)
    if query: 
        resultados= Product.objects.filter(Nombre = query)
        data = list(resultados.values('barcode','name','sale_price','count'))

    else :
        data=[] 
    return JsonResponse(data, safe= False)   ##RENDERCIZAR A MAIN        
